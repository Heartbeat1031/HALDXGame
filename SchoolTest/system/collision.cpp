	...
	namespace GE {
		...
		namespace Collision {
				...
				const Vector3& vecDistance)		// 中心座標を結んだベクトル
			{
				...
				float fDistance{};
				fDistance = fabs(vecDistance.Dot(vecSeparate));	// 中心座標を結んだベクトルと分離軸の内積を求める
				...
				if(fDistance > fShadowA + fShadowB) {	// 分離軸に射影した影の長さを求める
			...
			bool CollisionOBB(
				...
				Vector3 vecDistance = obbB.worldcenter - obbA.worldcenter;	// 中心座標を結ぶベクトル
				...
				// OBB-Aの3軸を分離軸にして判定
				for (int i = 0; i < 3; i++) {
					vecSeparate = *OBB_A_Axis[i];	// 分離軸を設定
				...
				// OBB-Bの3軸を分離軸にして判定
				for (int i = 0; i < 3; i++) {
					vecSeparate = *OBB_B_Axis[i];  // 分離軸を設定
				...
				for (int p1 = 0; p1 < 3; p1++) {
					for (int p2 = 0; p2 < 3; p2++) {
						...
						Vector3 obbAaxis = *OBB_A_Axis[p1];	// OBB-Aの軸
						Vector3 obbBaxis = *OBB_B_Axis[p2];	// OBB-Bの軸

						Vector3 crossseparate = obbAaxis.Cross(obbBaxis);	// 外積を求める
