#define NOMINMAX // 在包含 windows.h 之前加，防止它定? max/min 宏
#include <windows.h>
#include <algorithm>
...
namespace GM31 {
    namespace GE {
        ...
        namespace Collision {
            // OBBの重なりを判定する
            bool CompareLengthOBB(const BoundingBoxOBB &ObbA, const BoundingBoxOBB &ObbB, const Vector3 &vecSeparate,
                                  const Vector3 &vecDistance) {
                // ここを完成させる	(課題No3　start)
                float fDistance = fabs(vecDistance.Dot(vecSeparate));
                float fShadowA = fabs(vecSeparate.Dot(ObbA.axisX)) * (ObbA.lengthx * 0.5f)
                                 + fabs(vecSeparate.Dot(ObbA.axisY)) * (ObbA.lengthy * 0.5f)
                                 + abs(vecSeparate.Dot(ObbA.axisZ)) * (ObbA.lengthz * 0.5f);
                float fShadowB = fabs(vecSeparate.Dot(ObbB.axisX)) * (ObbB.lengthx * 0.5f)
                                 + fabs(vecSeparate.Dot(ObbB.axisY)) * (ObbB.lengthy * 0.5f)
                                 + fabs(vecSeparate.Dot(ObbB.axisZ)) * (ObbB.lengthz * 0.5f);
                return fDistance <= (fShadowA + fShadowB);
            }
            ...
            // OBBの当たり判定を行う
            bool CollisionOBB(const BoundingBoxOBB &obbA, const BoundingBoxOBB &obbB) {
                // ここを完成させる	(課題No3　start)
                Vector3 vecDistance = obbB.worldcenter - obbA.worldcenter;
                const Vector3 axesA[3] = {obbA.axisX, obbA.axisY, obbA.axisZ};
                const Vector3 axesB[3] = {obbB.axisX, obbB.axisY, obbB.axisZ};

                for (int i = 0; i < 3; ++i) {
                    if (!CompareLengthOBB(obbA, obbB, axesA[i], vecDistance)) {
                        return false;
                    }
                }
                for (int i = 0; i < 3; ++i) {
                    if (!CompareLengthOBB(obbA, obbB, axesB[i], vecDistance)) {
                        return false;
                    }
                }
                for (int i = 0; i < 3; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        Vector3 sepAxis = axesA[i].Cross(axesB[j]);
                        if (sepAxis.LengthSquared() < 1e-6f) {
                            continue;
                        }
                        sepAxis.Normalize();
                        if (!CompareLengthOBB(obbA, obbB, sepAxis, vecDistance)) {
                            return false;
                        }
                    }
                }
